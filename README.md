...
This system follows a strict architectural boundary: **The LLM orchestrates; the Python engine calculates.** The LLM (Gemini 1.5 Flash) interprets natural language prompts and generates structured JSON execution plans, while pure Python primitives execute deterministically without any LLM-side calculations.
...
* [app.py](./app.py): The main Streamlit dashboard workspace. Implements the UI layout, template shortcuts, metric displays, conversational chat panels, and handles user state management.
* [orchestrator.py](./orchestrator.py): The core orchestration engine. Parses user prompts (via Gemini Flash or local regex fallbacks) into JSON plans and executes these plans deterministically against the analytics engine.
* [analytics.py](./analytics.py): Mathematical primitives and vectorized routines for all calculations (normalization, z-scores, rolling PCA, correlation, relative strength, drawdowns, and regime detection).
* [db.py](./db.py): Interacts with the local caching system (DuckDB database + Parquet files) to load and save series using Polars.
* [ingest.py](./ingest.py): Multi-source data pipeline fetching from FRED (Federal Reserve Economic Data), Yahoo Finance, and Llama.fi (Stablecoin market caps).
* [visuals.py](./visuals.py): Altair visualization configurations, including the custom Bloomberg obsidian theme, layered charting configurations, and custom stress indicator visualizations.
* [prompts.py](./prompts.py): System prompt configurations defining constraints for structural JSON intent translation.
...
All quantitative transforms are defined in [analytics.py](./analytics.py):
* **Normalization & Scaling**: Normalizes series to `100.0` at specific event/anchor dates (`normalize_series`), rolling Z-scores (`zscore_series`), rolling percentile ranks (`percentile_scaling`), and robust Gaussian scaling.
...
* **Relationships & Factor Models**: Rolling Pearson correlations (`rolling_correlation`), divergence detection based on regression slope comparison (`detect_divergence`), and lead-lag cross-correlation analysis.
...
The ingestion pipeline ([ingest.py](./ingest.py)) fetches economic data and financial assets:
...
3. **Yahoo Finance assets**: Equities (`SPY`), Volatility (`^VIX`), Gold (`GLD`), Inflation-Protected Treasuries (`VTIP`), Long Bonds (`TLT`), Emerging Debt (`EMB`), Junk Bonds (`JNK`), and Semiconductor ETFs (`SOXX`).
...
- API Keys:
    ...
    - **Gemini API Key** *(Optional)*: Get one from [Google AI Studio](https://aistudio.google.com/) to enable conversational features. If omitted, the orchestrator defaults to a local regex parser.

### Installation

1. **Clone the repository**:
    ```bash
    git clone https://github.com/AJAX6255/randall-2-app.git
    cd randall-2-app
    ```
